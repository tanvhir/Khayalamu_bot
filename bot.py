...
# API Keys & Security (V3 এর সাথে ফুল এলাইন্ড)
...
# State & Memory (V3 ডিকশনারি + V4 গেটওয়ে লক স্টেটস)
user_data = {
...
# ৫৪ চ্যাপ্টারের এলাইন্ড ডিকশনারি (তোর পলিশ করা V3 ডেটা)
CHAPTER_NAMES = {
    ...
    "C1_C1": "ল্যাবরেটরির নিরাপদ ব্যবহার", "C1_C2": "গুণগত রসায়ন", "C1_C3": "মৌলের পর্যায়বৃত্ত ধর্ম ও রাসায়নিক বন্ধন", "C1_C4": "রাসায়নিক পরিবর্তন", "C1_C5": "कर्मমুখী রসায়ন",
    "C2_C1": "পরিবেশ রসায়ন", "C2_C2": "জৈব রসায়ন", "C2_C3": "পরিমাণগত রসায়ন", "C2_C4": "تড়িৎ রসায়ন", "C2_C5": "অর্থনৈতিক রসায়ন",
    ...
    "M2_C1": "বাস্তব সংখ্যা ও অসমতা", "M2_C2": "বহুপদী ও বহুপদী সমীকরণ", "M2_C3": "জटिल সংখ্যা", "M2_C4": "দ্বিপদী বিস্তৃতি",
...
# 🚀 SYSTEM PROMPT (জিতু ভাইয়ার ১০০% জেনুইন বাংলাদেশি পারসোনা + V4 সিকিউর ট্র্যাকিং রুলস)
SYSTEM_PROMPT = """
...
- Current Backlog Status: User has around 30+ lecture backlogs. Clear it step-by-step.
...
# --- Database Syncers (V3 এলাইন্ড ব্যাকহ্যান্ড থ্রেডিং) ---
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data.get("found"):
                ...
                logging.info("✅ Core Sync Completed Successfully!")
...
def generate_openrouter_chat(system_prompt: str, user_message: str, temperature: float = 0.7) -> str:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # ১. কাইজেন গোল মডিফিকেশন পার্সিং
            ...
            # ২. কাইজেন ডেইলি লাইফস্টাইল লগ পুশ
            ...
            # ৩. ডেইলি টার্গেট প্রপার স্ট্যাটাস পার্সিং
...
# --- UI Layout Dashboard Formatters (V3 পলিশড ড্যাশবোর্ড) ---
...
async def view_syllabus_tree(update: Update, context: ContextTypes.DEFAULT_TYPE, filter_arg=None):
    ...
    if not has_data: return await update.message.reply_text("এই ফিল্টারের আন্ডারে কোনো সিলেবাস ডেটা খুঁজে পাওয়া যায়নি।")
    msg += "━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += "(ফিল্টার করার অপশন: /report P বা /report P1_C6)"
    await update.message.reply_text(msg)

# --- Keyboard Menus (V4 সিকিউর স্টেট কন্ট্রোল লেআউট) ---
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        "কিরে ভাই, আমি তোর মেন্টর জিতু ভাইয়া।\n\n"
        "সেকেন্ড টাইমে কিন্তু ভুল করার বা ফালতু সময় নষ্ট করার বিন্দুমাত্র সুযোগ নাই। ২০২৬ সালের ডিসেম্বরের মিশন মাথায় রেখে জান লফিয়ে পড়তে হবে।\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ONE-TIME GATEWAY TRIGGERS (স্টেট গেটওয়ে ওপেনিং লক)
    ...
    elif text == 'Update Kaizen': user_data["current_state"] = "WAITING_FOR_KAIZEN_UPDATE"; return await update.message.reply_text("কালকের কাইজেন গোলের অবস্থা বল, সফল নাকি ব্যর্থ? কী হয়েছিল খুলে বল।")
    ...
    if state == "WAITING_FOR_TARGET":
        user_data["daily_target_raw"] = text; user_data["current_state"] = "NORMAL" # Reset Lock Immediately!
        save_target_to_sheet("Pending")
        logs_raw = json.dumps(user_data["kaizen_logs"][:5])
        sys_prompt = SYSTEM_PROMPT.format(current_time=get_bd_time().strftime("%I:%M %p"), daily_target_raw=text, kaizen_goals=user_data["kaizen_goals"], kaizen_logs_raw=logs_raw, context_reason="User has set a daily target. Motivate them in a sharp, short human-like style.")
        ...
        user_data["current_state"] = "NORMAL" # Reset Lock Immediately!
        logs_raw = json.dumps(user_data["kaizen_logs"][:5])
        sys_prompt = SYSTEM_PROMPT.format(current_time=get_bd_time().strftime("%I:%M %p"), daily_target_raw=user_data["daily_target_raw"], kaizen_goals=user_data["kaizen_goals"], kaizen_logs_raw=logs_raw, context_reason="PARSING_TARGET_UPDATE")
        ...
        user_data["current_state"] = "NORMAL" # Reset Lock Immediately!
        logs_raw = json.dumps(user_data["kaizen_logs"][:5])
        sys_prompt = SYSTEM_PROMPT.format(current_time=get_bd_time().strftime("%I:%M %p"), daily_target_raw=user_data["daily_target_raw"], kaizen_goals=user_data["kaizen_goals"], kaizen_logs_raw=logs_raw, context_reason="PARSING_KAIZEN_SET")
        ...
        user_data["current_state"] = "NORMAL" # Reset Lock Immediately!
        logs_raw = json.dumps(user_data["kaizen_logs"][:5])
        sys_prompt = SYSTEM_PROMPT.format(current_time=get_bd_time().strftime("%I:%M %p"), daily_target_raw=user_data["daily_target_raw"], kaizen_goals=user_data["kaizen_goals"], kaizen_logs_raw=logs_raw, context_reason="PARSING_KAIZEN_LOG")
        return await update.message.reply_text(generate_openrouter_chat(sys_prompt, text, 0.5), reply_markup=get_main_keyboard())

    # Syllabus Processing Sub-States
    elif state == "WAITING_FOR_ADD":
        mode, ch_key, lec_key = parse_smart_shortcode(text)
        if not ch_key or mode != "LECTURE": return await update.message.reply_text("কোড বা ফরম্যাট ঠিক নাই ভাই! উদাহরণ: P1 C6 L1")
        full_lkey = f"{ch_key}_{lec_key}"; full_ch_main = f"{ch_key}_CH"
        ...
        user_data["current_state"] = "NORMAL"; return await update.message.reply_text("সফলভাবে সিলেবাসে যুক্ত করা হয়েছে!", reply_markup=get_main_keyboard())
        ...
        if full_lkey in user_syllabus:
            ...
            user_data["current_state"] = "NORMAL"; return await update.message.reply_text("ক্লাস কমপ্লিট মার্ক করা হয়েছে!", reply_markup=get_main_keyboard())
        user_data["current_state"] = "NORMAL"; return await update.message.reply_text("এই লেকচারটি সিলেবাসে আগে যুক্ত করা হয়নি।", reply_markup=get_main_keyboard())
        ...
        task = state.split("_")[-1].lower(); full_ch_main = f"{ch_key}_CH"
        if full_ch_main not in user_syllabus: user_syllabus[full_ch_main] = {"note": "Pending", "practice": "Pending", "exam": "Pending"}
        user_syllabus[full_ch_main][task] = "Done"
        save_single_lecture_to_sheet(full_ch_main, **{f"{task}_val": "Done"})
        user_data["current_state"] = "NORMAL"; return await update.message.reply_text(f"চ্যাপ্টারের {task.capitalize()} সফলভাবে ডান মার্ক করা হয়েছে!", reply_markup=get_main_keyboard())

    # ------------------ NORMAL CHAT MODE ------------------
    # এই মুডে তুই স্বাধীনভাবে যা খুশি বল, শিটের কোনো ডেটা বা লগ উল্টাপাল্টা চেঞ্জ হবে না!
    logs_raw = json.dumps(user_data["kaizen_logs"][:5])
    sys_prompt = SYSTEM_PROMPT.format(current_time=get_bd_time().strftime("%I:%M %p"), daily_target_raw=user_data["daily_target_raw"], kaizen_goals=user_data["kaizen_goals"], kaizen_logs_raw=logs_raw, context_reason="Respond organically as a mentor. No logging triggers or secret tags are allowed here.")
    await update.message.reply_text(generate_openrouter_chat(sys_prompt, text, 0.7))

# --- Hourly Smart Reminder Engine (স্লিপ ফিল্টার ও অটো-স্টপ এলাইন্ড) ---
async def hourly_mentor_check(context: ContextTypes.DEFAULT_TYPE):
    ...
    # রাত ১২টা থেকে সকাল ৬টা পর্যন্ত স্লিপ মোড ফিল্টার
    if current_hour >= 0 and current_hour < 6: 
        return
        
    logs_raw = json.dumps(user_data["kaizen_logs"][:5])
    sys_prompt = SYSTEM_PROMPT.format(current_time=get_bd_time().strftime("%I:%M %p"), daily_target_raw=user_data["daily_target_raw"], kaizen_goals=user_data["kaizen_goals"], kaizen_logs_raw=logs_raw, context_reason="Hourly checking routine. Give a sharp 1-2 lines push toward goals.")
...
def main():
    ...
    print("✅ Jeetu Bhaiya AI V4 (Smarter State Engine) Running Successfully!")
    app.run_polling()
